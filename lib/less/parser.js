...
less.Parser = function Parser(env) {
    ...
    function chunkify( chunks, string ) {
        var j = 0,
            skip = /[^"'`\{\}\/\(\)]+/g,
            comment = /\/\*(?:[^*]|\*+[^\/*])*\*+\/|\/\/.*/g,
            level = 0,
        ...
        for (var i = 0, c, cc; i < string.length; i++) {
            skip.lastIndex = i;
            if (match = skip.exec(string)) {
                if (match.index === i) {
                    i += match[0].length;
                    chunk.push(match[0]);
                }
    ...
    return parser = {
        ...
        parsers: {
            ...
            directive: function () {
                var startIndex = i;
                var startChunk = j;
                var name, value, rules, types, params, iterated;
                if (input.charAt(i) !== '@') return;
                ...
                if (value = $(this['import'])) {
                    ...
                    // this is a psuedo directive that inserts an iterated set
                    // of styles into the unparsed chain, but it does not add anything
                    // directly to the tree.

                    //get iteration params: @iterate [replace] [from] [to]
                    params = ($(/^[^\{\r\n]+/) || '').trim().replace(/\s+/g," ").split(" ");

                    //use the standard parsers to figure out what we're going to iterate
                    var templateIndex = i;
                    var templateChunk = j;
                    content = $( this.block );
                    var endIndex = i;
                    ...
                    var template = input.slice(templateIndex+1,endIndex-1);

                    var result = [];
                    if ( params.length >= 3 && content){
                        var from = params[1];
                        var to = params[2];
                        var selector = params[3];
                        var r;

                        if (!isNaN(from) && !isNaN(to)){
                             var search = new RegExp(String(params[0]),'g');
                     		 if (selector != null){
                     		 	//perform selector iteration
                     		 	selector = selector.replace(/\"/g,"");
                     		 	var selectors = [];
                                for (r=from; r<=to; r++){
                                    selectors.push( selector.replace(search,r) );
                                }
                                result.push( template.replace(search, selectors.join(",")) );
                     		 }
                     		 else{
                                 //perform block iteration
                                 for (r=from; r<=to; r++){
                                      result.push( template.replace(search,r) );
                                 }
                     		 }
                        }
                    }
                    if (result.length > 0){
                        //update input string
                        var iinput = result.join('');
                        input = input.slice(0,startIndex) + iinput + input.slice(endIndex);
                        //update chunks
                        var ichunks = chunkify( [[]], iinput );
                        chunks = chunks.slice(0,startChunk).concat( ichunks, chunks.slice(endChunk) );
                    }
                    //move back to start indices and let the other parsers handle the rest!
                    i = startIndex;
                    j = startChunk;
